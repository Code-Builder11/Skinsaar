 ...
 * 
 * High-performance canvas-based scroll scrubbing of 300 sequential frames.
 * Zero external framework dependencies — pure vanilla JavaScript.
 */
...
(function (window, document) {
    ...
    class HeroScrollSequence {
        constructor(options = {}) {
            // Read optional data attribute from container for dynamic asset path resolution
            const containerEl = document.querySelector(options.containerSelector || '#hero-scroll-container');
            const dataPath = containerEl ? containerEl.dataset.sequencePath : null;

            // Resolve default base path ('assets/hero-sequence/' or 'hero-sequence/')
            const defaultBasePath = dataPath || (window.SKINSAAR_HERO_PATH || 'assets/hero-sequence/');
            const normalizedBasePath = defaultBasePath.endsWith('/') ? defaultBasePath : `${defaultBasePath}/`;
            ...
            this.config = Object.assign({
                ...
                imagePathPattern: (index, basePath) => `${basePath}ezgif-frame-${String(index).padStart(3, '0')}.jpg`,
                stages: [
                    { id: '1', enter: 0.00, leave: 0.28 },
                    { id: '2', enter: 0.32, leave: 0.62 },
                    { id: '3', enter: 0.66, leave: 0.98 }
                ]
            }, options);
            ...
            if (!this.container || !this.canvas) {
                console.warn('[HeroScrollSequence] Required canvas or container element not found.');
                return;
            }
            ...
            this.loadingQueue = [];
            this.fallbackPathAttempted = false;
        ...
        init() {
            // Setup canvas dimensions with DPR compensation
         ...
         * 1. Loads priority frames (1 to priorityCount) first for instant interactivity.
         * 2. Uses requestIdleCallback / chunked timeouts to stream remaining frames smoothly in background.
         */
        startPreloadPipeline() {
            ...
            const onPriorityFrameLoaded = (index) => {
                ...
                // Render frame 1 immediately as soon as it arrives
                if (index === 1 && this.lastRenderedFrame === -1) {
                    this.renderFrame(1);
                }

                // When priority buffer is complete, dismiss loader and queue background frames
        ...
        /**
         * Loads a single frame into memory with automatic path fallback
         */
        loadImage(index, callback) {
            ...
            const img = new Image();
            img.decoding = 'async';
            img.src = this.config.imagePathPattern(index, this.config.basePath);
            ...
            img.onerror = () => {
                // If primary path fails on frame 1 and fallback hasn't been tried yet, try alternative relative path
                if (!this.fallbackPathAttempted && (this.config.basePath.includes('assets/hero-sequence') || this.config.basePath.includes('hero-sequence'))) {
                    this.fallbackPathAttempted = true;
                    const altPath = this.config.basePath.includes('assets/') ? 'hero-sequence/' : 'assets/hero-sequence/';
                    console.info(`[HeroScrollSequence] Retrying with alternative path: ${altPath}`);
                    this.config.basePath = altPath;
                    img.src = this.config.imagePathPattern(index, this.config.basePath);
                    return;
                }

                console.warn(`[HeroScrollSequence] Failed to load frame ${index} from ${img.src}`);
                if (callback) callback(index);
            };
        }

        /**
         * Background frame streaming using requestIdleCallback / chunked timeouts
         */
        ...
        /**
         * Smart frame renderer: draws nearest loaded frame if exact target is buffering
         */
        requestRender() {
        ...
        renderFrame(frameIndex) {
            ...
            if (canvasRatio > imgRatio) {
                ...
                // Screen is taller than image aspect ratio (e.g. mobile portrait)
                dh = ch;
                dw = ch * imgRatio;
                dx = (cw - dw) / 2;
                dy = 0;
